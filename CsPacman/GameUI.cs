...
public class GameUI : Control, IRender
{
    ...
    private readonly Pen grid = new(Color.FromArgb(0x30, 0x30, 0x30));
    ...
    {
        this.state = state;
        status = $"Score: {state.score} Tick: {state.tick}";
        Invalidate();
    }
    ...
    protected override void OnPaint(PaintEventArgs e)
    {
        if (state != null)
        {
            ...
            {
                for (int j = 0; j < Level.Width; j++, idx++)
                {
            ...
            foreach (Characters.Character ghost in state.ghosts)
            {
